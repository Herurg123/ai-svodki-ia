...
поэтому редакционная дедупликация продолжает работать. Публичный `posts/`,
RSS, sitemap, FTP и исторический `posts/dzen-test/**` этот workflow не меняет.
...
- `repository-cleanup.yml` — ночная безопасная компактизация рабочих материалов старше 32 дней;
