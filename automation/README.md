...
Окно следующего запуска начинается от фактического `search_cutoff_at`
последнего успешно опубликованного выпуска и заканчивается новым pre-research
cutoff. `search_window.end_at` является авторитетным текущим временем. Системная
дата модели и UTC-дата среды исполнения не имеют права сдвигать редакционное
окно.
...
- `open_page` и `find_in_page` остаются диагностикой и не считаются search
  operation.
...
Fallback по-прежнему fail-closed: технически неполный audit блокирует Image
API, commit и deploy; те же правила действуют для `partial`,
`budget_exhausted` и `error`. Полностью завершённый поиск с нулевым итоговым
пулом становится зелёным `editorial_stop` без публикации.
...
Краткая формула совместимости production-контракта: **12 primary + до 4 hybrid +
до 7 coverage**. Теоретический worst case составляет **23** search operations.
Это не обычный дневной расход: полный выпуск после primary + hybrid не запускает
тяжёлый fallback, а четвёртый hybrid search выполняется только при полностью
пустом тематическом кластере.
...
повторяет уже оплаченные стадии без необходимости. В частности:
