# Step 4 evidence handoff — INDEPENDENT ACCEPTANCE FAILED
...
Independent Terra review of commit `1383da3c3718150a9cbc9164f9c44ad420524ca6`
failed. The original development checks below remain historical evidence, not
independent acceptance. Implementation was frozen after the finding.
No production PR/merge or paid production API call was performed.

## Independent result — FAIL

Terra produced a synthetic admissible counterexample: a saved 403 for a science
model can be marked resolved by a separate coding-model release from the same
company. The corrected control uses the real source failure, a recursively
schema-valid raw model response, the exact query generated by the code, the real
resolution accounting and then the normal source gate. The unrelated event is
accepted; `retrieval_quality.status=complete` with zero required signals left.
Both event timestamps fall inside the same window. This is an identity failure,
not a date-gate bypass or budget overrun: simulated Coverage usage is 7/7.

The original event has no title identity anchor beyond its company. The current
fallback accepts two shared title tokens, which generic same-company releases
can share. Source Freshness validates the alternative's own date, not its relation
to the lost event. Baseline does not contain this source-failure handoff, so the
new path introduces this false-close opportunity.

This is not an observed production wrong-story selection. The real Daybreak/403
incident demonstrates the route; the independent synthetic case demonstrates
its unsafe matching rule. The primary executor reproduced the corrected case.

Independent report/control/result: `2026-09-07-terra-independent-review.{md,py,json}`.
Prepared search inputs/protocol: `2026-09-07-terra-search/` — NOT EXECUTED. Both
live-search arms were stopped before their first call after the independent FAIL.
No implementation edits, PR, merge or paid production calls followed the finding.
The remaining full architecture matrix is not claimed as passed.
...
1. Correct event identity, preserve unresolved status when evidence does not bind
   the same event, and independently re-run this failure control. Then complete
   assistant-side Terra discovery/selection A/B using comparable inputs and the
...
The user explicitly authorized the separate Terra executor and conditional
PR/merge. The executor used `gpt-5.6-terra`, which matches the configured production
text model. The live search tool transport was not exercised because independent
architecture acceptance failed first. The user also explicitly prohibited automatic
continuation to step 5; steps 5–6 remain unstarted.
