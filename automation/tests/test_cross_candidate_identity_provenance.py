...
def test_sep3_real_shared_source_fixture_still_passes() -> None:
    fixture = load_fixture()
    assert fixture["shared_url"] in validator.candidate_identity_urls(
        fixture["candidates"]["candidates"][1]
    )
    assert validate(fixture)["errors"] == []


def test_foreign_identity_cannot_hide_inside_expected_supporting_sources() -> None:
    fixture = load_fixture()
    ...
    bad["stories"][0]["sources"] = [copy.deepcopy(google["supporting_sources"][0])]
    bad["article_html"] = bad["article_html"].replace(google_primary, shared, 1)

    report = validate(bad)

    assert codes(report) == ["story_source_identity_conflict"]


def test_expected_candidate_may_use_identity_shared_as_someone_elses_support() -> None:
    fixture = load_fixture()
    meta = fixture["candidates"]["candidates"][1]
    shared = fixture["shared_url"]

    assert meta["event_origin_url"] == shared
    assert validate(fixture)["errors"] == []


def test_unique_supporting_source_remains_valid_final_evidence() -> None:
    fixture = load_fixture()
    ...
    assert validate(changed)["errors"] == []


def test_shared_identity_owned_by_expected_candidate_is_not_rejected() -> None:
    fixture = load_fixture()
    changed = copy.deepcopy(fixture)
    google = changed["candidates"]["candidates"][0]
    ...
    changed["stories"][0]["sources"] = [copy.deepcopy(google["supporting_sources"][0])]
    changed["article_html"] = changed["article_html"].replace(google_primary, shared, 1)

    assert validate(changed)["errors"] == []


def test_unselected_candidate_identity_still_blocks_contamination() -> None:
    fixture = load_fixture()
    bad = copy.deepcopy(fixture)
    google = bad["candidates"]["candidates"][0]
    google_primary = google["primary_source"]["url"]
    ...
    bad["article_html"] = bad["article_html"].replace(google_primary, foreign_url, 1)
...
def test_pairwise_identity_contamination_matrix_fails_closed() -> None:
    size = 7
    candidates = []
    ...
    checked = 0
    for expected_index, expected in enumerate(candidates):
        for owner_index, owner in enumerate(candidates):
            ...
            owner_source = copy.deepcopy(contaminated[owner_index]["primary_source"])
