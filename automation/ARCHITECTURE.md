...
- общий локальный text-log path ротируется shared writers через `log-utils.js` до append: `worker.log` хранится 7 дней, error-log 30 дней, размерный порог 25 МБ; `dzen-browser-runner.js` намеренно остаётся вне этой миграции, а локальный `logs/.rotation-state.json` сохраняет логическую дату stream, чтобы его прямая запись не скрывала следующую day-boundary rotation;
