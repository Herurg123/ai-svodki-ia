...
> Эта рабочая ветка содержит НЕ ПРИНЯТЫЙ черновик пункта 4: URL и исходные
> временные доказательства rejection и отказов Source Freshness передаются в
> существующий resolution; начатый supplemental slot сохраняется до транспорта
> и не открывается повторно при восстановлении. Узкий OpenAI HTML + RSS adapter
> проверяет дату точной статьи. Это ещё не подтверждение поиска самой Terra.
> Бюджеты и публикационные gates не расширяются. Независимая приёмка Terra
> завершилась FAIL: generic title overlap может ошибочно закрыть source-failure
> сигнал другим событием. Terra search A/B не запускался. Production use запрещён
> до закрытия ограничений в
> `audits/experiments/2026-09-07-step04-evidence-draft.md`.
...
### Рабочая ветка пункта 4: контракт, ожидающий приёмки

`source_resolution_evidence.py` связывает отказ Source Freshness с исходным
прошедшим Primary cap кандидатом по заголовку и точному набору URL, а не по
переиспользуемому candidate ID. Сохраняется снимок до исключения; для старого
артефакта допустима только однозначная реконструкция из Primary того же окна.
Поздняя успешная проверка того же события отменяет старый сигнал. Надёжно старое
событие, неподтверждённый исходный кандидат и чужое окно не дают rescue-сигнал.
Исходный исключённый кандидат не получает include: альтернативный кандидат
снова проходит обычные freshness, dedupe и редакционные этапы.

`outside_window` остаётся диагностикой, если нет точной aware source timestamp,
попадающей внутрь окна. Дата без времени не становится таким доказательством.
Исходные URL, причины и время передаются существующему седьмому resolution;
имя конкретного события ограничивает объединение сигналов одной организации.
Шесть обязательных направлений, ceilings 24/25 и cap кандидатов не расширены.
Пропущенный для полного выпуска Coverage не запускается заново из-за сигнала.
Занятый седьмой слот может менять приоритет agency corroboration по существующей
quality policy; последствия выбора требуют отдельного Terra-эксперимента.

Перед вызовом resolution канонический report атомарно фиксирует attempted slot,
дату выпуска, окно и неопределённый результат. Ошибка транспорта сохраняет этот
маркер; миграция качества и отсутствие диагностических файлов не освобождают
оплаченный/неопределённый слот. Recovery копирует доказательства только из
выбранной lineage артефакта с совпадающим окном, без нового опроса источников.

Если generic metadata отсутствует, adapter для точного HTTPS OpenAI `/index/`
сопоставляет единственную видимую дату в article hero с единственным точным URL
в официальном RSS и aware `pubDate`. Нужны согласованные даты, отсутствие
редиректов и однозначная запись. Это один дополнительный публичный HTTP-запрос
RSS для подходящей страницы, без платного поиска. Generic metadata и независимый
event-age gate сохраняют приоритет. Повторная обычная source-проверка может снова
читать страницу/RSS; копирование recovery-диагностики таких запросов не делает.

Проверенные офлайн-сценарии и оставшиеся условия допуска перечислены в
`audits/experiments/2026-09-07-step04-evidence-draft.md`. Этот раздел описывает
черновик ветки, а не внедрённое поведение `main`.
...
primary URL. Syndication/aggregator URL не заменяет прямой источник. Rescue не
повышает significance и не гарантирует публикацию.
...
regional-health publication semantics или editorial policy. Yandex repair
ограничен corroborated first-party URL+visible-date evidence; same-day recovery
использует только сохранённый snapshot и не repoll'ит source. Tier B не получает
candidate influence, Tier-A trusted-news остаётся `consider` only после
deterministic gates.

Для P3 provider-routing dependency audit затрагивает Primary query contracts,
stable Hybrid query overlay, active agency rescue v5, same-day rescue recovery,
...
V4 сохраняется только для replay/rollback, active v5 Reuters route остаётся global
