...
- The first automated implementation of that route must remain a **dry-run**: it may create/resume a Dzen video draft, upload local MP4/PNG and fill metadata, but it must stop before the final `Опубликовать` click until a live dry-run is separately validated and promotion is explicitly approved.
- Preserve `videoEditorPublicationId` / draft URL in local state as soon as a Dzen draft is created. Re-runs must resume a known draft rather than silently create duplicate drafts when possible.
...
- The isolated dry-run implementation and operator procedure are documented in [`DZEN_NATIVE_UPLOAD_DRY_RUN.md`](DZEN_NATIVE_UPLOAD_DRY_RUN.md). Do not wire it into `worker.js`, `run-worker.cmd` or Task Scheduler in the same experimental change.
