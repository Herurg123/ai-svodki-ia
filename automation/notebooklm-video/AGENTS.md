...
- The 2026-08-27 controlled test disproved two proposed ways to attach video to an already imported Dzen article in this project: adding Media RSS `media:group`/MP4 metadata to the existing item, and adding an explicit HTML5 `<video>` block to that existing item's `content:encoded` even when the corresponding HTML page was also updated. A normal visible text marker in the same existing RSS item was imported by Dzen in about one minute, so these failures must not be explained merely as Dzen not re-reading the feed. Treat both video approaches as failed for the tested existing-item scenario unless new evidence materially changes the result.
- Any next Dzen video-delivery experiment must be isolated, documented before it is promoted to production behavior, and recorded in `DZEN_VIDEO_EXPERIMENTS.md` with observed results. The currently proposed next hypothesis is a separate new video publication/new RSS item; its exact format is not yet an approved contract.
