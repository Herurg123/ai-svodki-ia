...
- The 2026-08-27 manual Dzen Studio test and the 2026-08-28 automated MVP test confirmed that native browser upload works on the target Windows machine and lands in the Dzen video flow.
...
- The canonical live-publish baseline is a **fresh-upload, single-child, single-page MVP**: open Studio, create a new video upload, transfer the MP4, fill title/description once, set cover once, confirm exactly five tag chips, stop touching metadata/cover/tags, wait for the final processed/ready status, set comments to `Все пользователи`, click `Опубликовать`/`Отправить` once, log the click, then let the runner close the browser.
- A normal live run must not reopen or resume a saved `videoEditorPublicationId`, must not reopen old drafts, and must not refill already prepared metadata. If a run fails before the publish click, the next explicit operator run starts a new upload. Remote drafts are not auto-deleted.
...
- The validated MVP intentionally has no post-click verification and no automatic second click. Do not repeat the same run blindly after a logged successful publish click.
- The next isolated experiment may add a **pre-upload duplicate guard** by checking Studio → Publications → Video for the visible title prefix before ` | `. Do not promote that guard into the canonical baseline until it passes a live test.
