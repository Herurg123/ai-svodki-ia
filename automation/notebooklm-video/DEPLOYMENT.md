...
При первом запуске после обновления runtime автоматически создаётся `archive/` и выполняется безопасная компактация JSON-history: в active `state.json` и `_СКАЧАННЫЕ_ВИДЕО.json` остаётся 14-дневное окно плюс любые незавершённые jobs, а более старая безопасно завершённая история переносится в помесячные JSON-файлы. `archive/` не имеет auto-delete retention и при переносе машины должен сохраняться вместе с active state/registry. Текстовый `worker.log` проверяется на day/size rotation до append общими writers. Day-boundary определяется по timestamp уже записанных строк; отдельный rotation sidecar не требуется. Повторные запуски в один день должны накапливаться в одном active log.
