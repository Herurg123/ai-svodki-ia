...
Перед первой mutation editor Windows clipboard отдельно preflight-проверяется записью/чтением exact video URL. Если clipboard не работает, статья ещё не изменена и этап падает безопасно. Исходное содержимое Windows clipboard намеренно не читается, не сохраняется и не восстанавливается: runtime может оставить в clipboard video URL, использованный для реального Ctrl+V. Тем самым недоступный пользовательский clipboard до запуска браузера больше не блокирует весь article-video flow.
