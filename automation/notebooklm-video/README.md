...
Live-путь использует тот же защищённый Яндекс-профиль/CDP bootstrap, загружает MP4,
один раз заполняет metadata/обложку/5 тегов/комментарии и после этого больше не
переписывает описание. Один явный live-запуск выполняет один child-проход: при
ошибке runner не запускает полный publish-flow заново через несколько секунд, а
завершает команду и оставляет сохранённый draft/state для следующего отдельного
операторского запуска. Финальный клик разрешается только когда форма показывает
правый нижний статус `Загрузили и обработали видео` +
`Готово: можно публиковать и смотреть` и точная кнопка `Опубликовать` активна.
После клика сохраняется защита от повторного нажатия и проверяется увеличение
числа одноимённых записей именно во вкладке `Видео`.

Подробный операторский контракт: [`DZEN_NATIVE_UPLOAD.md`](DZEN_NATIVE_UPLOAD.md).
Практические результаты и неудачные гипотезы: [`DZEN_VIDEO_EXPERIMENTS.md`](DZEN_VIDEO_EXPERIMENTS.md).
...
Если `TargetDir` выбран другой, исключение должно указывать именно на него. Это
операторское требование: worker запускает Node.js, управляет браузером через
Playwright/CDP, использует PowerShell/DPAPI и `ffmpeg-static`, поэтому
поведенческие эвристики антивируса могут помещать отдельные runtime-файлы в
карантин или удалять их, оставляя локальный образ частично повреждённым.
...
[`DEPLOYMENT.md`](DEPLOYMENT.md), чтобы оно было видно и при локальном
развёртывании без чтения всего README.
...
- `НАСТРОЙКИ.txt` с локальной памяткой по конфигурации и обязательным
  предупреждением об антивирусном исключении;
