...
def _run_pre_hybrid_agency_rescue(
    *, artifact_dir: Path, archive_path: Path, publication_date: str, api_key: str,
    model: str, maximum_candidates: int, output_root: Path,
) -> dict[str, Any]:
    try:
...
def _pre_hybrid_source_freshness_gate(
    *, rescue: dict[str, Any], artifact_dir: Path, publication_date: str,
    ...
    if not rescue_rows:
        return rescue
    base_rows = [item for item in rows if item.get("audit_direction") != AGENCY_DISCOVERY_RESCUE_DIRECTION]
    window = research.get("search_window")
    result = copy.deepcopy(rescue)
    result["source_freshness_gate"] = {
        "version": 1, "status": "running", "paid_api_calls": 0,
    ...
    result["validated_count_before_source_freshness"] = int(rescue.get("accepted_count", rescue.get("added_count", 0)) or 0)
    ...
    _persist_rescue_report(result, artifact_dir=artifact_dir, output_root=output_root, publication_date=publication_date)
    return result
...
def _attach_rescue_to_hybrid_report(
    *, report: dict[str, Any], rescue: dict[str, Any], artifact_dir: Path,
    ...
    report["pre_hybrid_quality_search_operations"] = int(rescue.get("search_operation_count_contribution", 0) or 0)
    ...
    try:
        ...
        if isinstance(fusion_research, dict):
            refresh_post_hybrid_fusion(
                artifact_dir=artifact_dir, output_root=output_root,
                publication_date=publication_date, research=fusion_research,
            )
    except Exception:
        pass
    if pulse_path.is_file():
        try:
            ...
            report["source_pulse_shadow"] = {
                "version": 1, "status": "diagnostic_read_error",
                "candidate_influence": False, "paid_api_calls": 0,
    ...
    *, artifact_dir: Path, archive_path: Path, publication_date: str, api_key: str,
    ...
    if not gaps or maximum_search_calls < DEFAULT_MAXIMUM_SEARCH_CALLS:
        report = _BASE_RUN(
            artifact_dir=artifact_dir, archive_path=archive_path,
            publication_date=publication_date, api_key=api_key, model=model,
            maximum_search_calls=maximum_search_calls, maximum_candidates=maximum_candidates,
    ...
    report = _BASE_RUN(
        artifact_dir=artifact_dir, archive_path=archive_path,
        publication_date=publication_date, api_key=api_key, model=model,
        maximum_search_calls=FIXED_SEARCH_CALLS, maximum_candidates=maximum_candidates,
        request_fn=request_fn, output_root=output_root,
    )
    ...
    existing = [copy.deepcopy(item) for item in current_research.get("candidates", []) if isinstance(item, dict)]
    prompt = _regional_prompt(
        publication_date=publication_date, search_window=search_window,
        gaps=gaps, existing_candidates=existing, archive=archive,
    )
    ...
    record = _base._attempt_record(
        direction_id=ADAPTIVE_DIRECTION_ID,
        label="Regional recall health-check", prompt=prompt,
    ...
    report.update({
        ...
        "regional_health": {
            ...
            "gaps": list(gaps), "checked": checked,
            "query": regional_health_query(gaps),
