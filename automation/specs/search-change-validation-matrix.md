...
Контроль события без отдельного имени: два разных релиза одной организации
с общими словами заголовка, разными фактами, временем и source URL. Ответ о
втором релизе не должен закрывать Source Freshness 403 первого. Ветка пункта 4
этот независимый контроль пока НЕ проходит: см.
`audits/experiments/2026-09-07-terra-independent-review.py`. Это отрицательный
синтетический контроль, а не зафиксированная подмена в опубликованном выпуске.

## 1. Когда матрица обязательна

Контрольный инцидент для передачи source evidence: выпуск 2026-09-05, Daybreak,
Axios 403. Fixture `fixtures/recall/source-resolution-daybreak-2026-09-05.json`
разделяет исторический артефакт и отдельно полученное текущее OpenAI HTML/RSS.
Детерминированный replay `audits/experiments/replay_source_resolution_2026_09_07.py`
проверяет передачу исходного отказа, 144 комбинации объёма/региона/состояния
источника/порядка и сохранение занятого седьмого слота. Вручную заданная
альтернативная статья не доказывает, что поисковая модель найдёт или выберет её.
Для приёмки обязательны отдельные Terra discovery/selection A/B и независимая
оценка приоритета, caps, overlap, recovery и увеличения prompt/schema.
