...
Production recovery добавляет отдельный invariant над этими semantic rules. Если durable `processed` journal доказывает stale positive evidence-v1..v5 и exact provenance-bound candidate всё ещё присутствует в current research, reusable prior Coverage report или complete story snapshot, v7 выполняет deterministic preflight до `existing_full_digest` и `prior_complete` shortcuts. Preflight не переписывает optional-slot journal, удаляет только тот же exact stale provenance, сохраняет unrelated candidates, санирует reusable report и при затронутом publishable snapshot атомарно записывает `coverage-p3b-v7-revocation-<DATE>.json` со state `pending`, после чего убирает old `stories.json` из publishable artifact. `pending`/`blocked` переживают crash и не считаются завершёнными до clean rebuild; provider search, retry и page refetch preflight не открывает. Report-only stale provenance очищается offline без лишней инвалидизации чистого complete digest.
...
- crash после pending marker, включая ситуацию когда optional-slot journal позже отсутствует, не возвращает old complete snapshot; первый original SHA/backup metadata сохраняется;
- child failure оставляет marker `pending`; ложный child success, который вернул revoked story либо не создал новый `stories.json`, переводит marker в `blocked` и возвращает non-zero;
- current evidence-v6 positive не запускает v7 stale preflight;
- v7 preflight не выполняет ordinary/protected provider call, retry, Web Search или authoritative-page refetch и не меняет optional-slot journal bytes/search-budget consumption;
- report-only stale provenance может быть удалено offline без ненужного editorial rebuild чистого complete artifact;
...
V7 имеет отдельный publication/recovery marker state machine `pending → completed` либо `pending → blocked`; это не optional-slot state и не разрешение на новый provider call. Marker существует только для того, чтобы stale complete snapshot не мог снова стать publishable между sanitation и downstream rebuild.
...
V7 post-merge stale-positive reuse remediation is specified by `automation/P3B_V7_RECOVERY.md` and executable controls `automation/tests/test_p3b_v7_recovery_preflight.py`; it changes orchestration/recovery placement, not search-side semantics.
