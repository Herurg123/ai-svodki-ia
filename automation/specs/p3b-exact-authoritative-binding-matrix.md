...
Active P3b использует binder implementation `weak_source_exact_binding_v4.py` при сохранённом durable request-contract `VERSION=2`; public Coverage path идёт через v6 → v5 → v4 → v3 → v2 compatibility chain. Binder v4 отдельно маркирует semantic positive proof через `EVIDENCE_VERSION=5`, не разрывая durable request identity. Evidence v5 сохраняет все evidence-v4 fail-closed требования и дополнительно делает historical classification relation-aware: старый год в причине текущей отмены не маскирует current contradiction, quoted/curly wrappers не скрывают separate trailing `by` attribution, а genuinely historical passive lifecycle claim классифицируется как background до foreign-agent veto. Positive processed snapshot с более старым evidence marker, включая evidence-v4, не переиспользуется как current proof. P3b может рассмотреть максимум один qualified P3a weak-source signal и использует только уже существующий optional seventh Coverage slot. Шесть mandatory Coverage directions не меняются. Required high-signal `unverified` resolution имеет приоритет. Если optional capacity занята, потрачена или неоднозначна, weak-source signal остаётся unresolved/deferred. Восьмой Coverage search запрещён. Обычный whole-pipeline ceiling остаётся 24 Web Search operations; существующий conditional double-regional-gap ceiling остаётся 25.
...
- replacement trailing attribution нельзя спрятать несколькими wrapper/separator tokens: `((by OpenAI))`, `: — by OpenAI`, `{by OpenAI}`, `"by OpenAI"`, `({by OpenAI})` и аналогичные bounded combinations должны fail closed;
...
- current lifecycle contradiction нельзя маскировать unrelated historical marker: `launch cancelled today due to a 2025 incident` остаётся current non-positive contradiction, а не historical background;
...
- positive `processed` snapshot без current binder `EVIDENCE_VERSION=5`, включая evidence-v1/evidence-v2/evidence-v3/evidence-v4 positives, не переиспользуется как current hardened proof;
...
| `processed` | Reuse только current hardened result с `EVIDENCE_VERSION=5`; stale positive evidence-version fail-closed | 0 |
