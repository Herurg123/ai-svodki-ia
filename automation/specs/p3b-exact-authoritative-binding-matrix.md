...
- stale positive processed proof revokes только provenance-bound P3b candidate даже если current model/archive/request hash drift уже заставил preserved layer вернуть early deferred; cleanup является v6 postcondition, а не следствием совпадения current request identity;
...
- replacement trailing attribution нельзя спрятать несколькими wrapper/separator tokens: `((by OpenAI))`, `: — by OpenAI` и аналогичные bounded combinations должны fail closed;
...
- historical/background same-identity claim с foreign attribution, включая полную дату вроде `on September 1, 2025`, не veto'ит отдельную current exact claim; historical-only proof остаётся non-positive;
...
- positive `processed` snapshot без current binder `EVIDENCE_VERSION=4`, включая evidence-v1/evidence-v2/evidence-v3 positives, не переиспользуется как current hardened proof;
...
| `processed` | Reuse только current hardened result с `EVIDENCE_VERSION=4`; stale positive evidence-version fail-closed | 0 |
...
Preserved `ensure_story_coverage_p3a.py` должен оставаться byte-identical pre-P3b public implementation. Historical binder v1/v2/v3 остаются forensic/compatibility assets; binder v4 наследует hardened v3 semantics, сохраняет durable `VERSION=2`/mode и отдельно использует `EVIDENCE_VERSION=4` для current positive processed proof. Evidence-v1/evidence-v2/evidence-v3 positive snapshots считаются stale и fail-closed без нового provider search/page refetch. Stale semantic-proof cleanup является v6 postcondition: current request-hash/model/archive drift может запретить replay, но не может сохранить candidate, admitted только obsolete proof.
