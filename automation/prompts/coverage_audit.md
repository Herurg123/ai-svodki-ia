...
2. Фактический search query должен быть короткой natural-language фразой,
   ориентированной на свежие новости effective window, обычно 6–18 значимых
   слов. Календарные даты указывай обычным текстом. **Не используй `after:`,
   `before:`, `site:`, длинные `OR`-цепочки, скобки и перечисление десятков
   доменов или компаний.** Если для прохода уже задан API domain filter, не
   дублируй разрешённые домены в query: фильтр существует именно для этого.
3. Для `general_coverage_gaps` не пытайся вручную превратить весь список
   `DIRECTION_ALLOWED_DOMAINS` в `site:foo OR site:bar ...`. Используй короткий
   запрос вида `AI major news products business infrastructure August 11 2026
   August 12 2026`; API сам ограничит выдачу авторитетными last-mile доменами.
4. Открой подходящие страницы: сниппет, Reddit или социальная сеть могут быть
   только сигналом, но не единственным подтверждением итогового кандидата.
5. Приоритет: первоисточники; Reuters/AP и сопоставимые агентства; профильные
   security, legal, научные и технологические издания; затем прозрачные
   вторичные источники с указанными ограничениями.
6. Событие и основной источник должны попадать в строгое окно. Если источник
...
7. Старый материал, перепечатанный сегодня без нового развития, не является
...
8. Не возвращай уже найденное событие, полный дубль архива, слух, вымышленный
   запуск модели, мелкое обновление, рекламу или материал, где ИИ — приманка.
9. Для `legal_copyright_scraping` кандидат с recommendation include/consider
...
10. Для `curiosity` кандидат обязан иметь `category: "curiosity"`,
...
14. Верни только строгий JSON по заданной схеме.
