...
2. Первые 24 часа effective window являются healing overlap предыдущего выпуска.
   Для фактического search query сначала вычисли основной continuity-период:
   он начинается ровно через 24 часа после `{{SEARCH_WINDOW_START_AT}}` и
   заканчивается в `{{SEARCH_WINDOW_END_AT}}`. Поисковая строка должна в первую
   очередь ранжировать новости этого основного периода. Overlap остаётся
   допустимой областью для случайно обнаруженного крупного пропуска, но не должен
   вытеснять более свежие события.
3. Фактический search query должен быть короткой natural-language фразой,
   обычно 6–18 значимых слов. Календарные даты **основного continuity-периода**
   указывай обычным текстом. **Не используй `after:`, `before:`, `site:`, длинные
   `OR`-цепочки, скобки и перечисление десятков доменов или компаний.** Если для
   прохода уже задан API domain filter, не дублируй разрешённые домены в query:
   фильтр существует именно для этого.
4. Для `general_coverage_gaps` не пытайся вручную превратить весь список
   ...
   source-neutral запрос вида `AI major news products business infrastructure
   August 13 2026 August 14 2026`, где даты относятся к основному continuity-
   периоду; API сам ограничит выдачу авторитетными last-mile доменами.
