...
В этой рабочей ветке сохранён НЕ ПРИНЯТЫЙ черновик пункта 4 архитектуры:
передача доказательств после Source Freshness, точная дата OpenAI и сохранение
занятого поискового слота при восстановлении. Офлайн-проверки пройдены,
но независимая приёмка Terra завершилась FAIL: другое событие той же компании
может ошибочно закрыть source-failure сигнал. Поисковой Terra A/B остановлен
до запуска. PR/merge не выполнены.
Статус, проверки и ограничения:
`automation/audits/experiments/2026-09-07-step04-evidence-draft.md`.
