...
5. Для пула короче обычной цели выполняется coverage audit: шесть отдельных
   ...
   седьмой слот становится `high_signal_recall_sentinel`: компактным поиском
   по Reuters, AP, Bloomberg и Financial Times на предмет явно значимого
   пропущенного ИИ-события. Предел audit остаётся 7 завершённых операций
   `search`, общий дневной потолок — 19. Служебные `open_page` и
   `find_in_page` сохраняются в траектории, но не расходуют search-бюджет.
...
high-signal recall sentinel по крупнейшим агентствам. Успешный sentinel
сохраняется в `coverage-audit.json` и recovery не повторяет его; технически
незавершённый sentinel делает нулевую редакционную остановку непригодной и
может быть повторён следующим recovery-run без повторения шести завершённых
направлений.
...
завершённого основного research, полного coverage audit и требуемого для
нулевого пула recall sentinel итоговый пул всё ещё пуст, workflow создаёт
...
- если не найдено ни одного достаточно подтверждённого сюжета после полного
  обязательного поиска и zero-pool recall sentinel, публикация не создаётся;
...
- завершённый research с нулевым первичным пулом остаётся пригодным для
  ...
  coverage audit;
- полный coverage audit и завершённый zero-pool recall sentinel переиспользуются
  без новых запросов; частичный audit продолжает только ещё не завершённые
  направления, а технически не завершённый sentinel может использовать
  свободный седьмой слот повторно;
