...
5. Для пула короче обычной цели выполняется coverage audit: шесть отдельных
   ...
   седьмой слот становится `high_signal_recall_sentinel` версии 2: одним
   компактным Reuters-only Web Search с API-фильтром `reuters.com`. Такой
...
Reuters-only high-signal recall sentinel v2. Prompt требует один содержательный
