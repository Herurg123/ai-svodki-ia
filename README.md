...
5. Для пула короче обычной цели выполняется coverage audit: шесть отдельных
   ...
   седьмой слот становится `high_signal_recall_sentinel` версии 5: одним
   компактным Reuters-only Web Search без API domain filter; Reuters задаётся
   прямо в тексте короткого запроса. Такой режим появился после регрессии, где
   более широкий четырёхдоменный sentinel формально завершался, но уходил в
   старые материалы и пропускал свежую Reuters-новость. Предел audit остаётся
   7 завершённых операций `search`, общий дневной потолок — 19. Служебные
   `open_page` и `find_in_page` сохраняются в траектории, но не расходуют
   search-бюджет.
...
Reuters-only high-signal recall sentinel v5. Prompt требует один содержательный
поисковый запрос без `site:` и `OR`. Для sentinel API-доменный фильтр намеренно
не используется после подтверждённого false-zero; Reuters фиксируется в самом
коротком запросе. Успешный sentinel сохраняется в `coverage-audit.json` вместе
с версией и recovery не повторяет его. Нулевой artifact с отсутствующей или
устаревшей версией sentinel возобновляется: шесть завершённых проходов
сохраняются, а освобождённый седьмой слот расходуется только на актуальную
recall-проверку.
...
- полный coverage audit и завершённый sentinel текущей версии переиспользуются
  ...
  заменяется одним актуальным Reuters-only проходом без повторной оплаты шести
