...
5. Для пула короче обычной цели выполняется coverage audit: шесть отдельных
   ...
   компактным Reuters-only Web Search с без API domain filter, с Reuters в тексте короткого запроса. Такой
   режим появился после регрессии, где более широкий четырёхдоменный sentinel
   формально завершался, но уходил в старые материалы и пропускал свежую
   Reuters-новость. Предел audit остаётся 7 завершённых операций `search`,
   общий дневной потолок — 19. Служебные `open_page` и `find_in_page`
   сохраняются в траектории, но не расходуют search-бюджет.
...
поисковый запрос без `site:` и `OR`, а домен ограничивается самим Web Search
API. Успешный sentinel сохраняется в `coverage-audit.json` вместе с версией и
recovery не повторяет его. Нулевой artifact с отсутствующей или устаревшей
версией sentinel возобновляется: шесть завершённых проходов сохраняются, а
освобождённый седьмой слот расходуется только на актуальную recall-проверку.
