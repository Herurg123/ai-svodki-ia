...
календарные дни допустимы: следующее окно исследования начинается с последнего
успешно опубликованного выпуска, а не обязательно с предыдущей даты.
...
5. Для пула короче обычной цели выполняется coverage audit: шесть отдельных
   ...
   направления. Предел audit — 7 вызовов, общий дневной потолок — 19.
...
Статусы `partial`, `budget_exhausted` и `error` сохраняются в диагностике, но
не отменяют уже готовый выпуск хотя бы с одним достойным сюжетом.
