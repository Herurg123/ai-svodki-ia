...
For Dzen article-video, `ERROR` is terminal by default. A scheduled run may perform at most one automatic recovery only for a proven pre-edit link-resolution error with no resolved links and no editor/publish markers; all post-edit or publish uncertainty stays manual-only. The local video runtime must not precreate unused `temp/` or `traces/` directories.
