...
The local NotebookLM-video runtime keeps bounded active JSON state: the current 14-day window remains in active `state.json` and the download registry, while older safe terminal history moves to the ignored local `archive/` directory. Rotated text logs also use that same local `archive/`, but text-log cleanup is restricted to `worker-*.log`/`error-*.log` and must never delete JSON-history. Unresolved jobs must never be archived solely because of age, archive history must remain readable for explicit old-date operations and duplicate checks, and the JSON archive is not subject to text-log retention. Shared text-log writers rotate before append through the video subproject logging utility. Rotation is derived from timestamps already present in the active log, not from mutable mtime or sidecar state, so repeated runs on the same day must never rotate away earlier same-day entries. `dzen-browser-runner.js` remains an explicit temporary direct-writer exception and must not be modified merely to normalize logging without owner approval; when it writes the first line of a new day into an older active log, the shared logger must preserve that current-day suffix while archiving only the older prefix.

For Dzen article-video, `ERROR` is terminal by default. Scheduled recovery is restricted to two one-shot fail-closed classes: proven pre-edit link-resolution with no resolved links/editor/publish markers, or pre-publish clipboard-related failure with resolved links but no publish/terminal markers and mandatory live-draft inspection before any new mutation. Any publish-arm/click uncertainty stays manual-only. The local video runtime must not precreate unused `temp/` or `traces/` directories.
